...
Ext.define('GeoExt.form.field.GeocoderComboBox', {
    ...
    findMapPanel: function() {
        var mapPanel = Ext.getCmp("mapPanelID");//this.up('gx_mappanel');
        if (mapPanel) {
    ...
    handleSelect: function(combo, rec) {
        ...
        if (Ext.isArray(value)) {
            var mapProj = mapPanel.map.getProjectionObject();
            delete this.center;
            ...
            if (value.length === 4) {
                ...
                mapPanel.map.setCenter(
                    OpenLayers.LonLat.fromArray(value)
                        .transform(this.srs, mapProj),
                    Math.max(mapPanel.map.getZoom(), this.zoom)
                );
            }
            rec = rec[0];
            this.center = mapPanel.map.getCenter();
