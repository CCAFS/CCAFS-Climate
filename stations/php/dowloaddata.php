...
if (isset($_REQUEST['station'])){$statList=json_decode($_REQUEST["station"]);}
if (isset($_REQUEST['variable'])){$varList=json_decode($_REQUEST["variable"]);}
if (isset($_REQUEST['typedwn'])){$typedwn = $_REQUEST["typedwn"];}
...
//print_r($varList);
  $where .= " AND station_variable_id IN (" . implode(",",$varList).")";
   ...
   if(!$dbcon){
	...
	if(isset($typedwn)){if($typedwn=='selection'){$outfile = "ccafs-climate_stations_sel.zip";}}
	if(isset($typedwn)){if($typedwn=='query'){$outfile = "ccafs-climate_stations_query.zip";}}
